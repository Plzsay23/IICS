...
####################################################### 수정 가능 #######################################################
target_accuracy = 88.0  # 사용자 편의에 맞게 조정
global_round = 30   # 사용자 편의에 맞게 조정
batch_size = 32  # 사용자 편의에 맞게 조정
num_samples = 1   # 사용자 편의에 맞게 조정
...
def measure_accuracy(global_model, test_loader):
    ...
    ######################### [핵심 1] Pruning (가지치기) #########################
    # Conv2d와 Linear 레이어의 가중치 중 절대값이 작은 30%를 0으로 만듦
    # GPU 상에서 마스킹 연산을 수행하므로 장치 이동 없음
    for name, module in model.named_modules():
        if isinstance(module, (nn.Conv2d, nn.Linear)):
            prune.l1_unstructured(module, name='weight', amount=0.3) # 30% Pruning
            prune.remove(module, 'weight') # 마스크를 영구 적용하여 모델 구조 확정
    #############################################################################

    ######################### [핵심 2] FP16 Quantization (GPU) ##################
    # Int8 양자화는 CPU 연산이 강제되는 경우가 많으므로, 
    # GPU 속도를 극대화하기 위해 Half Precision(FP16)을 사용합니다.
    model.half()  # 모델 전체를 FP16으로 변환
    #############################################################################
    ...
    # 추론 속도 측정을 위한 동기화 (정확한 GPU 시간 측정)
    if device == 'cuda':
    ...
    # 모델 크기 측정 (FP16 적용된 상태)
    # state_dict를 저장하여 압축된 크기를 확인
    temp_path = "temp_model.pth"
    torch.save(model.state_dict(), temp_path)
    final_model_size = os.path.getsize(temp_path) / (1024 ** 2)
    os.remove(temp_path)
    
    print(f"Optimized Model Size (FP16+Pruned): {final_model_size:.4f} MB")

    # 다음 라운드 전송을 위해 다시 float32로 돌려줄 필요가 있다면 변환, 
    # 여기서는 성능 측정용이므로 그대로 리턴
