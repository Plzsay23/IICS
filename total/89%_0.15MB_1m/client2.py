...
train_transform = v2.Compose([
    v2.Resize(IMG_SIZE, antialias=True),     # 192x192로 리사이즈
    v2.RandomRotation(degrees=20),            # ±20도 회전만
...
def main():
    ...
    for name, param in model.named_parameters():
        # 파라미터 이름에 'fc'가 포함되어 있으면(헤드) 학습(True), 아니면(백본) 고정(False)
        if "fc" in name:
            param.requires_grad = True
        else:
            param.requires_grad = False
    
    # Optimizer에는 'requires_grad=True'인 파라미터(헤드)만 전달해야 함
